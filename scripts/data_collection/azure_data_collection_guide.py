...
class AzureDataCollectionGuide:
    ...
    def create_azure_collection_guide(self):
        """Create comprehensive Azure data collection guide."""
        
        current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        guide = f"""# Azure Cloud Spend Data Collection Guide
...
"""
    ...
    def create_azure_cli_scripts(self):
        """Create Azure CLI scripts for automated data collection."""
        
        scripts = {
            "get_cost_data.ps1": """# PowerShell script to collect Azure cost data
...
Write-Host "Collecting storage account details..."
$storageAccounts = Get-AzStorageAccount
$storageDetails = $storageAccounts | Select-Object Name, ResourceGroupName, Location, 
    @{Name="Sku";Expression={$_.Sku.Name}},
    @{Name="Kind";Expression={$_.Kind}},
    @{Name="Tags";Expression={$_.Tags | ConvertTo-Json -Compress}}

$storageDetails | Export-Csv -Path "$outputDir/azure-storage.csv" -NoTypeInformation

Write-Host "Storage details exported to $outputDir/azure-storage.csv"

Write-Host "Data collection complete! Check the $outputDir folder for CSV files."
""",

            "get_utilization_data.ps1": """# PowerShell script to collect Azure resource utilization data
...
foreach ($vm in $vms) {
    ...
    $memoryMetrics = Get-AzMetric -ResourceId $vm.Id -MetricName "Available Memory Bytes" -StartTime (Get-Date).AddDays(-30) -EndTime (Get-Date) -TimeGrain 01:00:00
    
    if ($memoryMetrics.Data) {
        $memoryAvg = ($memoryMetrics.Data | Measure-Object -Property Total -Average).Average
        $memoryMax = ($memoryMetrics.Data | Measure-Object -Property Total -Maximum).Maximum
        $memoryMin = ($memoryMetrics.Data | Measure-Object -Property Total -Minimum).Minimum
    } else {
        $memoryAvg = $memoryMax = $memoryMin = 0
    }
    ...
    $utilizationData += [PSCustomObject]@{
        ...
        MemoryUtilizationAvg = [math]::Round($memoryAvg, 2)
        MemoryUtilizationMax = [math]::Round($memoryMax, 2)
        MemoryUtilizationMin = [math]::Round($memoryMin, 2)
...
""",
            ...
            "get_ri_recommendations.ps1": """# PowerShell script to get Reserved Instance recommendations
# Run this script to collect RI recommendations
...
# Create output directory
$outputDir = "azure-ri-data"
New-Item -ItemType Directory -Force -Path $outputDir

Write-Host "Collecting Reserved Instance recommendations..."

# Get RI recommendations (requires Azure Advisor API)
# Note: This is a simplified version - you may need to use Azure Advisor REST API

$riData = @()

# Get all VMs that could benefit from RIs
$vms = Get-AzVM

foreach ($vm in $vms) {
    # Calculate potential RI savings (simplified)
    $vmSize = $vm.HardwareProfile.VmSize
    $location = $vm.Location
    
    # This is a placeholder - actual RI recommendations require Azure Advisor API
    $riData += [PSCustomObject]@{
        ResourceId = $vm.Id
        ResourceName = $vm.Name
        ResourceGroup = $vm.ResourceGroupName
        Location = $location
        VMSize = $vmSize
        CurrentPricingModel = "On-Demand"
        RecommendedRI = "1 Year"
        PotentialSavings = "Calculate using Azure Advisor"
        UtilizationRequirement = "24/7"
        Notes = "Check Azure Advisor for actual recommendations"
    }
}

# Export RI data
$riData | Export-Csv -Path "$outputDir/azure-ri-recommendations.csv" -NoTypeInformation

Write-Host "RI recommendations exported to $outputDir/azure-ri-recommendations.csv"
Write-Host "Note: Actual RI recommendations should be obtained from Azure Advisor"
"""
        }
    ...
    def generate_azure_guide(self):
        ...
        print(f"   - get_ri_recommendations.ps1")
